...
export default function HeroOverlayText({ scrollProgress, onOpenBooking }) {
  ...
  return (
    <div className="absolute inset-0 z-20 pointer-events-none">
      ...
      {/* PHASE 1: BRAND INTRO (0.00 - 0.24) - POSITIONED CLEARLY BELOW NAVBAR */}
      <div
        className={`absolute inset-x-4 top-24 sm:top-28 flex flex-col items-center text-center max-w-xl mx-auto transition-all duration-700 ease-out ${
          isPhase1
            ? 'opacity-100 translate-y-0 pointer-events-auto scale-100'
            : 'opacity-0 -translate-y-6 pointer-events-none scale-95'
        }`}
      >
        <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/80 backdrop-blur-md border border-black/8 mb-3 shadow-sm">
          <span className="w-1.5 h-1.5 rounded-full bg-[#C5A059]" />
          <span className="font-sans text-[10px] tracking-[0.25em] text-[#8F7238] uppercase font-semibold">
            Nagpur Atelier
          </span>
        </div>
        ...
        <p className="font-sans text-[#524C44] text-xs sm:text-sm font-normal max-w-xs sm:max-w-md mb-4 leading-relaxed">
        ...
        <div className="flex flex-col items-center gap-1 text-[#8E867B] font-sans text-[10px] tracking-widest uppercase font-semibold animate-bounce">
          <span>Scroll down to deconstruct</span>
          <ArrowDown className="w-3.5 h-3.5 text-[#C5A059]" />
        </div>
      </div>

      {/* PHASE 2: METALLURGY (0.24 - 0.50) - CLEAR OF NAVBAR */}
      <div
        className={`absolute inset-x-5 top-28 sm:top-auto sm:inset-auto sm:left-14 sm:top-1/2 sm:-translate-y-1/2 max-w-xs sm:max-w-sm transition-all duration-700 ease-out mx-auto sm:mx-0 ${
          isPhase2
      ...
      >
        <div className="flex items-center gap-2 text-[#8F7238] font-sans text-[10px] tracking-[0.2em] uppercase mb-1.5 font-semibold">
        ...
        <div className="flex items-center gap-4 text-xs font-sans">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[#8E867B]">Comfort</span>
          </div>
        </div>
      </div>

      {/* PHASE 3: ZEISS OPTICS (0.50 - 0.74) - CLEAR OF NAVBAR */}
      <div
        className={`absolute inset-x-5 top-28 sm:top-auto sm:inset-auto sm:right-14 sm:top-1/2 sm:-translate-y-1/2 max-w-xs sm:max-w-sm transition-all duration-700 ease-out mx-auto sm:mx-0 sm:text-right ${
          isPhase3
            ? 'opacity-100 translate-y-0 sm:translate-x-0 pointer-events-auto scale-100'
            : 'opacity-0 translate-y-4 sm:translate-x-8 pointer-events-none scale-95'
        }`}
      >
        <div className="flex items-center gap-2 text-[#8F7238] font-sans text-[10px] tracking-[0.2em] uppercase mb-1.5 font-semibold sm:justify-end">
        ...
        <div className="flex items-center gap-4 text-xs font-sans sm:justify-end">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[#8E867B]">Shield</span>
          </div>
          <span className="text-black/20">|</span>
          <div className="flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-[#059669]" />
      ...
      <div
        className={`absolute inset-x-4 top-28 sm:top-1/2 sm:-translate-y-1/2 flex flex-col items-center text-center max-w-sm sm:max-w-md mx-auto transition-all duration-700 ease-out ${
          isPhase4
            ? 'opacity-100 scale-100 pointer-events-auto translate-y-0'
            : 'opacity-0 scale-95 pointer-events-none translate-y-4'
        }`}
      >
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/80 backdrop-blur-md border border-black/8 mb-3 shadow-sm">
          <Sparkles className="w-3.5 h-3.5 text-[#C5A059]" />
          <span className="font-sans text-[9px] tracking-[0.2em] text-[#8F7238] uppercase font-semibold">
